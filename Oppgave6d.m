...
intervals = ones(11,1);
exact_y = ones(11,1);
e_mach_mult=ones(11,1);
theo_error = ones(11,1);
...
%     h=L/n; %H�yde
    ...
    for k = 1:11
        n=10*(2^k);
        e_mach_mult(k,1) = condest(lagA(n))*eps;
        
        theo_error(k,1) = (2/n)^2;
        y=lagY3(n);
        num_y(k,1) = y(n);
        error(k,1) = abs(y(n)-a);
        intervals(k,1) = n;
    ...
    plot(log(intervals),log(error),log(intervals),log(e_mach_mult),log(intervals),log(theo_error));
%  axis([0 11 0.12 0.2])
end
